...
namespace twitch {
    ...
    bool AuthServer::authenticateUser(TokenPair& tokenPair) {
    ...
    bool AuthServer::authenticateApp() {
    ...
    bool AuthServer::validateToken(std::string token) {
    ...
    void AuthServer::generateState(const std::vector<std::string> scopes) {
    ...
    bool AuthServer::handleRequest(http::request<http::string_body>& request, ip::tcp::socket& socket) {
    ...
    std::map<std::string, std::string> AuthServer::parseHttpHeaders(boost::core::basic_string_view<char>& request) {
    ...
    void AuthServer::refererCatcher() {
    ...
    TokenPair AuthServer::generateTokens() {
    ...
    TokenPair AuthServer::firstAuth(const std::vector<std::string> scopes) {
    ...
    TokenPair AuthServer::refreshUserAccessToken(std::string refresh) {
        cpr::Response response = cpr::Post(cpr::Url{"https://id.twitch.tv/oauth2/token"},
            cpr::Payload{
            {"client_id", Tokens::fetchInstance().clientId},
            {"client_secret", Tokens::fetchInstance().clientSecret},
